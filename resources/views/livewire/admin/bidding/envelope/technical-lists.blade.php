<div id="technicals-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" wire:ignore.self
  ...
  <div class="relative p-4 w-full max-w-7xl max-h-full">
    ...
    <div class="relative bg-white rounded-lg shadow">
      ...
      <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
        ...
        <button type="button" wire:click="closeTechnicalModal"
      ...
      <form wire:submit="addTechnicals">
          ...
          <tbody>
            @forelse ($technicals as $technical)
              <tr class="bg-white border-b " wire:key="technical-{{ $technical->id }}">
                ...
                <td class="px-6 py-4 truncate text-xs">
                  {{ strtoupper($technical->question_type) }}
                </td>
                <td class="px-6 py-4">
                  <div class="flex items-center me-4">
                    <input wire:model.live="checkTechnicals.{{ $technical->id }}" value="{{ $technical->id }}"
                      type="checkbox"
        ...
        <div class="flex justify-end p-4 md:p-5  rounded-b">
          <button type="submit"wire:loading.remove wire:target="addTechnicals"
...
@script
  <script>
    $wire.on('openTechnicalModal', () => {
      var modalElement = document.getElementById('technicals-modal');
      var modal = new Modal(modalElement, {
        backdrop: 'static'
      });
      modal.show();
    });
    $wire.on('closeTechnicalModal', () => {
      var modalElement = document.getElementById('technicals-modal');
      var modal = new Modal(modalElement);
      modal.hide();
    });
