<div class="overflow-x-auto shadow-md sm:rounded-lg">
  <table class="w-full text-sm text-left rtl:text-right text-gray-500 boder-collapse">
    <caption class="p-5 text-lg font-semibold text-left rtl:text-right tei-text-secondary bg-white ">
      Project Bid Award
      <p class="mt-1 text-sm font-normal text-gray-500 ">List of all bids that are already approved and ready to be
        awarded.
      </p>
      ...
      @foreach ($tableHeader as $index => $header)
        <td scope="col" class="px-4 py-3 border-gray-100 border-e text-gray-900">
          <div class="flex justify-between">
            <span>{{ $header }}</span>
            @if ($index != 'winner' && $index != 'action' && $index != 'rank')
    ...
    <tbody>
      @forelse ($biddings as $bidding)
        <tr class="text-xs bg-white border-b font-extrabold hover:bg-neutral-200 hover:shadow-xl transition duration-300">
          ...
          <td class="px-6 py-2">
            @php
              $winner = $bidding->winnerApproval->winnerVendor;
            @endphp
            {{ $winner->name }}
          </td>
          <td class="px-6 py-2">
            @php
              $result = $bidding->finalResult->where('vendor_id', $winner->id)->first();
              $rank = $result ? $result->rank : null;
            @endphp
            <span class="{{ $rank == 1 ? 'text-green-500' : 'text-yellow-500' }}">{{ $rank }}</span>
          </td>
          <td class="px-6 py-2">
            @if (roleAccessRights('review'))
              <button wire:click.prevent="awardModal({{ $bidding->id }})" wire:loading.remove
                wire:target="awardModal({{ $bidding->id }})"
                class="hover:scale-110 transition-transform duration-300 mr-4 bg-green-500 rounded-md px-2 py-1 text-white text-xs">Award</button>
        ...
        @if ($selectedBidReview)
          <div class="mx-5 my-4">
            <div>
              ...
              <span class="text-xs uppercase font-extrabold tei-text-accent">{{ $selectedBidReview->title }}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span
                class="text-xs uppercase font-extrabold tei-text-accent">{{ $selectedBidReview->scrap ? 'Yes' : 'No' }}</span>
            </div>
          </div>
              ...
              <tbody>
                @foreach ($vendorResults as $vendor)
                  <tr class="hover:shadow-lg hover:bg-neutral-300 border-b">
                    ...
                    @foreach ($vendor['envelopes'] as $vendorEnvelope)
                      <td class="px-6 py-2 normal-case">
                        @if ($vendorEnvelope)
                          <span><i class="fa-solid fa-circle-check text-green-500"></i></span>
                        @else
                    ...
                    <td class="px-6 py-2 normal-case">
                      @if ($vendor['score'])
                        {{ $selectedBidReview->score_method == 'Rating' ? number_format($vendor['score'], 1) . '%' : 'PHP ' . number_format($vendor['score'], 2) }}
                      @else
                        <span><i class="fa-solid fa-circle-xmark text-red-500"></i></span>
                      @endif
                    </td>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow">
        ...
        <div class="p-4 md:p-5 text-center" wire:loading.remove wire:target="awardWinner">
          ...
          <h3 class="mb-5 text-lg font-normal tei-text-primary ">Are you sure you want to award the bid(<span
              class="font-semibold underline">{{ $selectedBidForAward ? $selectedBidForAward->title : '' }}</span>) to
