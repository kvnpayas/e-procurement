<div class="overflow-x-auto shadow-md sm:rounded-lg">
  <table class="w-full text-sm text-left rtl:text-right text-gray-500">
    <caption class="p-5 text-lg font-semibold text-left rtl:text-right tei-text-secondary bg-white ">
      Financial Groups
      <p class="mt-1 text-sm font-normal text-gray-500 ">Assign financials to group.
      </p>
    ...
    <thead class="text-xs text-gray-700 uppercase tei-bg-light ">
      <tr>
        ...
        <th scope="col" class="px-6 py-3">
          <div class="flex justify-between">
            <span>No. of financials</span>
            <button wire:click.prevent="selectedFilters('financials_count')">
              <div class="flex flex-col pt-0.5 text-gray-900">
                <i class="fa-solid fa-sort-up {{ $orderBy == 'financials_count' && $sort == 'desc' ? 'tei-text-secondary' : '' }}"
                  style="line-height: 0"></i>
                <i class="fa-solid fa-sort-down {{ $orderBy == 'financials_count' && $sort == 'asc' ? 'tei-text-secondary' : '' }}"
                  style="line-height: 0"></i>
              </div>
            </button>
          </div>
          ...
          <td class="px-6">
            <div class="flex gap-4">
              @if (roleAccessRights('update'))
                <div>
                  <button data-tooltip-target="tooltip-edit-{{ $group->id }}" type="button"
                    ...
                    wire:click.prevent="editModal({{ $group->id }})" wire:loading.remove
                wire:target="editModal({{ $group->id }})">
              ...
              @if (roleAccessRights(['create', 'update']))
                <div>
                  <button
                    wire:click="$dispatch('financialsModal', {id: {{ $group->id }}})"
  ...
  {{-- Financials Lists --}}
  @livewire('envelope-maintenance.financial-lists')
  {{-- End Financials Lists --}}
</div>
@script
  <script>
    $wire.on('closeModal', () => {
