...
<body>
  ...
  <div class="content-text">
    ...
    <p>Greetings!</p>
    <p>
      This is to inform all interested bidders that the bidding process for <span
        style="text-decoration: underline; font-weight:bolder">{{ $bidding->project_id }} - {{ $bidding->title }}</span>
      previously placed on hold on {{ date('F j, Y, g:i A', strtotime($bidding->hold_date)) }} is resuming effective
      {{ date('F j, Y, g:i A', strtotime(now())) }}.
    </p>
    <p>
      All previously submitted bids will be reconsidered, and new bids will be accepted until {{ $bidding->extend_date ? date('F j, Y, g:i A', strtotime($bidding->extend_date )) : date('F j, Y, g:i A', strtotime($bidding->deadline_date )) }}.
    </p>
    <p>
      Please note that all bid requirements, terms and condition remains unchanged unless otherwise specified in the Terms of Reference (TOR)/  Scope of Work (SOW)/ Supplemental Bid Bulletin.
    </p>
    <p>
      Thank you for your understanding and patience. Should you have any questions or require further information, please do not hesitate to contact us directly.
    </p>
    <p>
      Respectfully,
    </p>
