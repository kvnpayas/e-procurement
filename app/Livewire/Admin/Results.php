...
class Results extends Component
{
  ...
  public function getVendors()
  {
    ...
    $vendors = $getJoinedVendors->get()->map(function ($vendor) {
      ...
      $winner = $vendor->winBids->where('bidding_id', $this->bidding->id)->first();
    ...
    if ($this->bidding->financial) {
      $vendors = $vendors->map(function ($vendor) {
        $vendorFinancials = $this->bidding->financialVendors->where('vendor_id', $vendor->id);
        $totalAmount = 0;
        foreach ($vendorFinancials as $financial) {
          $finacnialData = $this->bidding->financials->where('id', $financial->financial_id)->first();
          $quantity = $finacnialData ? $finacnialData->pivot->quantity : 0;
          $price = $financial ? $financial->price : 0;
          $price = $financial ? $financial->other_fees : 0;
  ...
  {
    $rankWithData = AllResultsReport::getAllResults($this->bidding->id, $this->vendors);
    $this->dispatch('bidPackageModal', $this->bidding->id, $rankWithData);
  }
