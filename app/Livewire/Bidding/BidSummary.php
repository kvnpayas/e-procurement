...
class BidSummary extends Component
{
  ...
  public $project;
  public $results = [], $eligibilities, $enableSubmit, $vendorStatus;
  ...
  public function getResults()
  {
    ...
    $this->enableSubmit = in_array("0", $allEnvelopesStatus, true) ? false : true;
  ...
  public function getTechnicals()
  {
    ...
    if ($envelopeStatus && $envelopeStatus->status) {
      foreach ($this->project->technicals as $technical) {
        ...
        $files = $vendorInitResponse->vendorAttachments($this->project->id, $vendorInitResponse->technical_id)->get();

        $vendorFiles = $files->isEmpty() ? [] : $files->pluck('file')->toArray();
  ...
  {
    return view('livewire.bidding.bid-summary');
  }
}
